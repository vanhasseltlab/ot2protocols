...
#MAIN FUNCTION---------
main <- function(file_path, file_name, outputDir){
  #READ PLATE------
  ...
  #Command List-------
    #adjusting file name
  file_name <- strsplit(file_name, split='.xl', fixed=T)[[1]][1]
  outputName <- paste('CmdList_', file_name, '.csv', sep='')
    #precursors: amount list
  write.table(c(">Amount List"), file=paste(outputDir, outputName, sep='\\'), sep=',', col.names=F, row.names=F)
  write.table(allAmt, file=paste(outputDir, outputName, sep='\\'), sep=',', col.names=F, row.names=F, append=T)
    #main command list bulk
  rownames(cmdList) <- c()
  write.table(c('>CommandLines'), file=paste(outputDir, outputName, sep='\\'), sep=',', append=T, col.names=F, row.names=F)
  write.table(cmdList, file=paste(outputDir, outputName, sep='\\'), sep=',', append=T, col.names=F, row.names=F)
  
  #User Commands-----------
    #adjusting file name
  outputName <- paste('DeckGuide_', file_name, '.xlsx', sep='')
    #deckMap
  write.xlsx(finDeck, paste(outputDir, outputName, sep='\\'), sheetName='Deck Map', col.names=F, row.names=F)
    #amount list
  allAmt <- rbind.data.frame(allAmt, dilTubes)
  write.xlsx(allAmt, paste(outputDir, outputName, sep='\\'), sheetName ='Solutions and Tubes', append=T, row.names=F)
}
#TEST AREA-----------
#file
#homeDir <- "C:\\Users\\Sebastian\\Desktop\\MSc Leiden 2nd Year\\##LabAst Works\\FinVersion\\OT2_UpstreamExcelProc_MAIN\\250920200"
#inptFile <- "PlateMapInpt.xlsx"
#filePath <- paste(homeDir, "\\", inptFile, sep='')

#test
#main(filePath, inptFile, homeDir)
