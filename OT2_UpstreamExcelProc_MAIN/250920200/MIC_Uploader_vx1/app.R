...
Cmd_InitDist <- function(deck_map, sol_list, solvent_map, dil_map){
    ...
    for(j in c(1:length(solvents))){
        cur_sol_list <- subset(sol_list, Solvent==solvents[j])
        ...
        for(i in c(1:length(cur_sol_list[,1]))){
            ...
            nexCmd <- c(names(deck_map)[match('Solvent', deck_map)], #source ware = solvent rack
                        solvent_map[solvent_map[,2]==cur_sol_list$Solvent[i],1], #source slot = solvent tube
                        dil_map$Labware[dil_map$Fill==cur_sol_list$SolID[i]], #target ware = dilution rack
                        dil_map$Slot[dil_map$Fill==cur_sol_list$SolID[i]], #target slot = dilution tube
...
main <- function(file_path, file_name){
    ...
    cmdList_output <<- list(c(">Amount List"), allAmt,
...
server <- function(input, output) {
    ...
    #sourceName <- "UpstreamProcessor_FuncLib_v3.R"
    #sourceName <- paste(homeDir, sourceName, sep='\\')
    #source(sourceName)
    ...
    output$d_OT2 <- downloadHandler(
        filename = function(){paste("CommandList_", file_name, '.csv', sep='')},
        content = function(file) {
            write.csv(new_cmdLineOutput, file, row.names = FALSE)
        }
    )
    output$guide <- downloadHandler(
        filename = function(){paste("RobotHandler_", file_name, '.xlsx', sep='')},
        content = function(file) {
