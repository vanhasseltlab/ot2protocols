...
cmd_FirstDWDilution <- function(plate_map, deck_map, drug_info, solutions_map, cmd_list, well_info, serial_step, n_plates){
  #first dilution
  tipID <- as.numeric(max(cmd_list$tipID)) + 1
  solIDs <- unique(subset(plate_map, DrugName!='NA')$solID)
  ...
  for(i in c(1:length(solIDs))){
    ...
    reqVolume <- (serial_step/(serial_step-1))*((n_plates*finAmt_per_wells)+200)
...
cal_ReqSolutions <- function(solutions_map, plate_map, cmd_list, deck_map){
  ...
  filled_locs$Vol[filled_locs$Labware==toString(deck_map[grepl("tock",deck_map[,2]),1])] <- ceiling(filled_locs$Vol[filled_locs$Labware==toString(deck_map[grepl("tock",deck_map[,2]),1])]/100)*100 + 200
...
cal_amtList_Excess <- function(amt_list, cmd_list, deck_map){
    ...
    if(cmd_list$SourceLabware[i] %in% tubes$Labware){
      if(cmd_list$SourceSlot[i] %in% tubes$Slot[tubes$Labware == cmd_list$SourceLabware[i]]){
        ...
        if(volUsed_after <= 45000){
          ...
          #place new tube
          solvent_map <- subset(tubes, Labware==toString(deck_map[grepl('olvent', deck_map[,2]),1]))
  ...
  #round up; add excess
  amt_list$Vol[amt_list$Labware==toString(deck_map[grepl('olvent',deck_map[,2]),1])] <- ceiling(amt_list$Vol[amt_list$Labware==toString(deck_map[grepl('olvent',deck_map[,2]),1])]/1000) + 2
  amt_list$Vol[amt_list$Labware==toString(deck_map[grepl('olvent',deck_map[,2]),1])] <- sapply(amt_list$Vol[amt_list$Labware==toString(deck_map[grepl('olvent',deck_map[,2]),1])], function(x) min(x, 49000))
  ...
  amt_list$Vol[amt_list$Labware==toString(deck_map[grepl('tock',deck_map[,2]),1])] <- sapply(amt_list$Vol[amt_list$Labware==toString(deck_map[grepl('tock',deck_map[,2]),1])], function(x) min(x, 1350))
...
cmd_FillOuterWells_DeepWell <- function(deck_map, n_plate, cmd_list, 
  ...
  #iterate through all solvet types
  for(i in c(1:length(solvents))){
...
#USER INPUTS---------
#home <- "C:\\Users\\Sebastian\\Desktop\\MSc Leiden 2nd Year\\##LabAst Works\\FinVersion\\OT2_UpstreamExcelProc_MAIN\\Checkpoint - Postpatch\\20201103_6PlateSetUp" 
home <- "/home/sebastian/20201103_6MICtest"
setwd(home)
...
#WRITE---------
#write.csv(user_cmd, 'UserCommands2.csv', row.names=F)
#write.csv(OT2_cmd, 'OT2Commands2.csv', row.names=F)
